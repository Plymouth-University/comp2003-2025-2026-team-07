...
// PUT /api/auth/users/:id/pager-id - Update user pager ID
router.put('/users/:id/pager-id', authenticateToken, async (req, res, next) => {
  try {
    const targetId = parseInt(req.params.id);
    const { pager_id } = req.body;

    // Users can only update their own pager ID; admins can update anyone's
    if (req.user.id !== targetId && req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Forbidden' });
    }

    if (pager_id === undefined) {
      return res.status(400).json({ error: 'pager_id is required' });
    }

    // Validate: alphanumeric, max 20 chars (matches frontend pattern)
    if (pager_id && !/^[a-zA-Z0-9]{1,20}$/.test(pager_id)) {
      return res.status(400).json({ error: 'pager_id must be alphanumeric and at most 20 characters' });
    }

    const user = await prisma.users.update({
      where: { id: targetId },
      data: { pager_id: pager_id || null },
      select: { id: true, username: true, email: true, role: true, pager_id: true }
    });

    res.json({ success: true, message: 'Pager ID updated successfully', data: user });
  } catch (error) {
    next(error);
  }
});
