...
const pagemRoutes = require('./routes/pagem');
...
async function startServer() {
  try {
    ...
    app.listen(PORT, () => {
      ...
      // Load Pagem API key from DB (takes priority over env), then validate and start poller
      pagemService.init().then(() => {
        if (pagemService.enabled) {
          pagemService.testApiKey()
            .then(r => console.log('📟 Pagem API key validated:', r.message))
            .catch(err => console.warn('⚠️  Pagem API key validation failed:', err.message));
          pagemService.startStatusPoller();
        } else {
          console.log('📟 Pagem paging disabled (no API key configured)');
        }
      });
