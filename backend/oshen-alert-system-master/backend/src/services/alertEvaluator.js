...
const compoundAlertService = require('./compoundAlertService');
...
class AlertEvaluator {
  ...
  async createOrUpdateAlertHistory(alertRule, vessel, alertText, telemetryTimestamp) {
    ...
    if (existingAlert) {
      ...
      // Notify vessel supervisors via Pagem (fire-and-forget, don't block alert creation)
      pagemService.notifyVesselSupervisors(vessel.id, alertText)
        .then(async (results) => {
        ...
        .catch(err => console.error(`❌ Pagem notification error for alert #${newAlert.id}:`, err.message));
  ...
  async evaluateTelemetry(telemetryId) {
    try {
      ...
      // Check compound rules for this vessel after individual alerts are resolved
      const compoundResults = await compoundAlertService.evaluateCompoundRules(vessel.id);
      const compoundTriggered = compoundResults.filter(r => r.triggered).length;
      if (compoundTriggered > 0) {
        console.log(`🔗 ${compoundTriggered} compound alert(s) triggered`);
      }
      console.log('');
      ...
      return {
        ...
        compoundAlertsTriggered: compoundTriggered
      };
